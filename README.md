...
- **`src/chap_gis/cli/`** — concrete CLI commands that use the functionality of the package, exposed as the `chap-gis` console script.
...
```
  uv run chap-gis analyze --country=RWA
```
...
```
  uv run chap-gis visualize data/outputs
```
...
```
  uv run chap-gis aggregate data/outputs RWA 2
```
